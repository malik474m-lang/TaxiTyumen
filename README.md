...
`ServerHosting/admin/` повторяет структуру исходной `TaxiAdmin`: Дашборд, Заказы,
Водители (создание и полный профиль), Клиенты, Операторы/диспетчеры (создание,
схемы оплаты и смены), Балансы, Тарифы, Статистика, CSV, Автодозвон и
серверный Брендинг с загрузкой PNG/JPEG/WebP-логотипов.
