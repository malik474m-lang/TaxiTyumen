...
export default function OperatorApp({ branding }: { branding: BrandingData }) {
  ...
  if (!user || user.role !== "operator" || !user.token) {
    return <LoginScreen role="operator" onAuthed={setUser} branding={branding} />;
  }
  ...
  return (
    <div
      ...
      style={{ "--brand": branding.primaryColor, "--brand-ink": branding.primaryTextColor } as React.CSSProperties}
    >
      <AppHeader user={user} subtitle={branding.appCode} branding={branding} />
