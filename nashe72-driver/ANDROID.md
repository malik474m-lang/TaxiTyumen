# nashe72-driver для Android — полная инструкция по сборке APK
...
4. В PowerShell (в папке проекта):
   ```powershell
   dotnet build -t:Run -f net10.0-android nashe72-driver/nashe72-driver.csproj
   ```
...
- **Можно вообще без своего ключа (быстрый старт):** пропустите ШАГ 5 и в ШАГЕ 6
  ...
  `dotnet publish nashe72-driver/nashe72-driver.csproj -f net10.0-android -c Release -p:AndroidPackageFormat=apk`
...
```powershell
dotnet publish nashe72-driver/nashe72-driver.csproj -f net10.0-android -c Release `
...
```
C:\TaxiTyumen\nashe72-driver\bin\Release\net10.0-android\publish\ru.nashe72.driver-Signed.apk
```

> Однострочный вариант (если редактор «съедает» обратные кавычки-переносы):
> ```
> dotnet publish nashe72-driver/nashe72-driver.csproj -f net10.0-android -c Release -p:AndroidPackageFormat=apk -p:AndroidKeyStore=true -p:AndroidSigningKeyStore=taxi-driver.keystore -p:AndroidSigningStorePass=ВАШ_ПАРОЛЬ -p:AndroidSigningKeyAlias=taxi-driver -p:AndroidSigningKeyPass=ВАШ_ПАРОЛЬ
> ```

## ШАГ 7. Раздача водителям

1. Отправьте файл `ru.nashe72.driver-Signed.apk` водителям — в Telegram,
...
1. Поднимите версию в `nashe72-driver/nashe72-driver.csproj`
...
Бэкенд: `https://nashe72.ru/api/` (`Services/ApiService.cs`).
