...
**Пробки и озвучка в приложении водителя.** Ключ TomTom Traffic
(developer.tomtom.com — 2500 запросов/сутки бесплатно) укажите в админке
«API-ключи» → TomTom Traffic или константой `TOMTOM_API_KEY` в `config.local.php`.
Приложение водителя получает URL слоя через `/api/map-config.php` (`traffic.tileUrl`)
и кнопкой «Пробки» накладывает загруженность дорог поверх карты. Там же кнопка
«Озвучка»: голосовые подсказки манёвров (`/api/route.php?steps=1`, OSRM)
