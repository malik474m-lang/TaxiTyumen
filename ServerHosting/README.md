...
**Ключи можно не прописывать в файле:** админка → раздел **«API-ключи»**
(виден супер-администратору) хранит их в базе, и значение из базы имеет
приоритет над `config.local.php`. Файл остаётся резервным вариантом.

OpenCage (бесплатный триал: 2500 запросов/сутки, https://opencagedata.com/)
работает вторым источником после DaData: цепочка подсказок **DaData → Photon/OSM → Яндекс → OpenCage**
с дедупликацией результатов по координатам. Если ключ не задан — провайдер
просто пропускается, остальная система не меняется.

**Несколько аккаунтов OpenCage.** В разделе «API-ключи» можно указать сразу
несколько ключей (по одному в строке) — суточный лимит складывается
(N × 2500 запросов). При ответе `402 quota exceeded` ключ автоматически
исключается до ближайшей полуночи UTC, запрос тут же повторяется следующим
ключом; `429` даёт паузу 2 минуты, `401/403` — час. Состояние каждого ключа
(в работе / квота исчерпана / до какого времени, счётчики) видно в таблице
на той же странице, там же кнопка «Снять блокировки».
...
## Сервисы TomTom (админка → «TomTom»)

Один ключ из кабинета **MyTomTom** (developer.tomtom.com) обслуживает все сервисы:
бесплатно **50 000 тайлов и 2 500 остальных запросов в сутки**, коммерческое
использование разрешено. Ключ вносится в «API-ключи» (или `TOMTOM_API_KEY`
в `config.local.php`), а каждый сервис **включается и выключается отдельно**
в разделе «TomTom». Выключенный сервис не вызывается вообще — система работает
на прежних источниках (OSRM, DaData, Photon, Яндекс, OpenCage, тайлы OSM).

| Сервис | Тип | Что даёт |
|---|---|---|
| Traffic Flow | тайлы | Пробки на карте водителя (кнопка «Пробки») |
| Traffic Incidents | тайлы | ДТП, ремонт, перекрытия (кнопка «ДТП») |
| Map Display | тайлы | Базовая карта TomTom вместо OSM (офлайн-кеш — только с OSM) |
| Routing | запросы | Маршрут и ETA с живыми пробками вместо OSRM (+ манёвры на русском) |
| Search | запросы | Подсказки адресов в цепочке «Геокодинг» |
| Reverse Geocoding | запросы | Адрес по координатам в той же цепочке |
| Matrix Routing | запросы | Время подачи водителей с учётом пробок |
| Snap to Roads | запросы | Привязка GPS-трека к дорогам (честный километраж) |
| Reachable Range | запросы | Изохроны: куда водитель доедет за N минут |
| Static Image | тайлы | Статичная картинка карты для отчётов |

Расход нетайловых запросов считается в `tomtom_usage` и виден в админке;
при исчерпании суточного лимита сервисы автоматически уходят в паузу
до полуночи UTC, а система возвращается к OSRM и прежним геокодерам.
Состояние слоёв приложение получает через `/api/map-config.php`,
управление — `GET/PUT /api/tomtom.php` (admin).

**Ограничение ключа по Referer.** Если в MyTomTom ключ привязан к домену
(«Allowed referrers»), сервер передаёт домен в заголовке Referer каждого
запроса (константа `PUBLIC_BASE_URL` в `config.local.php`, по умолчанию
`https://taxi.event72.ru`) — добавьте его в список разрешённых либо снимите
ограничение. В приложении водителя тайлы TomTom proxy-ируются через локальный
сервер на устройстве (такой же, как тайлы OSM): Referer подставляется там же,
плюс файловый кеш с коротким TTL бережёт суточный тайловый лимит
(пробки/ДТП — 5 минут, базовая карта TomTom — 30 дней).

**Озвучка маршрута.** Кнопка «Озвучка» в приложении водителя: голосовые
подсказки манёвров (`/api/route.php?steps=1` — TomTom Routing или OSRM)
штатным синтезатором речи устройства, работает и без интернета.

**Navigation SDK для Android не подключается намеренно:** пользовательское
соглашение TomTom исключает «Navigation Functionality» из обычного плана
(нужен отдельный письменный договор), а для .NET MAUI потребовалась бы
binding-обёртка нативного Kotlin-SDK. Те же возможности закрыты REST-API
выше — в рамках бесплатного тарифа и без изменения архитектуры приложения.
