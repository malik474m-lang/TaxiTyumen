...
Готовый веб-интерфейс управления (server-rendered PHP, без Node.js):

| Страница | Что делает |
|---|---|
| `/admin/login.php` | Вход админа (cookie-сессия на HMAC, 12 ч) |
| `/admin/index.php` | Дашборд: выручка, заказы, графики по дням/часам, топ направлений |
| `/admin/orders.php` | Все заказы, фильтр по статусу, отмена, CSV-выгрузка |
| `/admin/drivers.php` | Водители: баланс, пополнение, верификация, история транзакций |
| `/admin/tariffs.php` | Редактор тарифов (цены, множители, комиссия) |
| `/admin/branding.php` | Внешний вид 3 приложений с живым превью |
| `/admin/autocall.php` | Настройки автодозвона |
| `/admin/export.php` | CSV по cookie-авторизации (без Bearer) |

Откройте `https://ваш-домен.ру/admin/` — редирект на форму входа.
Демо-вход: `+79001234567` / `Admin123!` (смените через `/api/auth/password.php`).
