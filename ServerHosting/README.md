...
Из коробки задействованы:

- **Matrix → автоназначение заказов.** `AutoCall::tick` набирает кандидатов
  по прямой (как раньше), а победителя выбирает по реальному времени подачи
  с учётом пробок. Matrix выключен/не ответил — поведение прежнее.
- **Snap → честный километраж.** `api/drivers/track.php` отдаёт `distanceKm`
  по дорогам (плюс `distanceByGpsKm`, `snapped`, `snappedTrack` для линии на
  карте), `admin/driver-track.php` показывает обе цифры, `actual_distance`
  в карточке завершённого заказа тоже считается по дорогам
  (на цену это НЕ влияет — информационное поле).
- **Reachable Range → зоны в один клик.** Админка «Зоны и цены» получила
  блок «Изохрона»: адрес (или центр города) + время в минутах → полигон
  живой доступности подставляется прямо в форму новой зоны — остаётся
  назвать и сохранить.
...
Эквайринг ИП, карточные платежи/СБП C2B, привязки карт, внутренний счёт,
безналичные кошельки и ручные выплаты самозанятым: [`SBER.md`](SBER.md).
...
`git pull` обновляет сайт автоматически. **Внимание:** сам корневой `.htaccess`
при пулле не меняется — если файл менялся в репозитории, скопируйте его в
корень домена ещё раз (иначе старые правила продолжат работать).
