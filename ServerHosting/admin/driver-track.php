...
$sql='SELECT * FROM driver_location_history WHERE driver_id=?';$params=[$id];if($orderId!==''){$sql.=' AND order_id=?';$params[]=$orderId;}$sql.=' ORDER BY timestamp DESC LIMIT 1000';$s=$db->prepare($sql);$s->execute($params);$points=array_reverse($s->fetchAll());
$distInfo=TomTom::trackDistance($db,array_map(fn($p)=>[(float)$p['latitude'],(float)$p['longitude']],$points));
$distanceText=number_format($distInfo['km'],2).' км'.($distInfo['snapped']?' по дорогам (по точкам GPS: '.number_format($distInfo['kmGps'],2).' км)':'');
...
?>
<div class="flex between"><div><h1>GPS-трек · <?=h($driver['first_name'].' '.$driver['last_name'])?></h1><p class="mut"><?=h($driver['license_plate'])?> · точек <?=count($points)?> · <?=h($distanceText)?></p></div><a class="btn ghost" href="drivers.php">← Водители</a></div>
