...
$admin = admin_require($db, 'tariffs');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db->prepare(
        'UPDATE tariffs SET name=?, description=?, base_fare=?, price_per_km=?, price_per_minute=?,
         ...
         free_waiting_minutes=?, paid_waiting_per_minute=?, preorder_surcharge=?,
        ...
        (float) ($_POST['preorder_surcharge'] ?? 0),
  ...
  <form method="post" class="card">
    ...
    <table style="margin-top:14px">
      <?php foreach ([
          ...
          ['preorder_surcharge', 'Наценка за предзаказ, ₽', $t['preorder_surcharge'] ?? 0],
