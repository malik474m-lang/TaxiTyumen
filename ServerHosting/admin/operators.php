...
$operators = $db->query(
    "SELECT u.*, p.scheme, p.rate_per_order, p.rate_per_hour, p.rate_per_day, p.fixed_monthly
     ...
     WHERE u.role='operator' ORDER BY u.last_name, u.first_name"
)->fetchAll();
...
<div class="flex between">
  <div><h1>Операторы / диспетчеры</h1><p class="mut"><?= count($operators) ?> учётных записей</p></div>
  <span class="chip info">Активны: <?= count(array_filter($operators, fn($o) => $o['is_active'] && !$o['is_blocked'])) ?></span>
</div>
