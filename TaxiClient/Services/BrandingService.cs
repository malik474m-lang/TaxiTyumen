...
/// Брендинг приложения клиента: название сервиса, цвета и логотип приходят
/// с сервера из админки («Брендинг» app=client и «Бренд сервиса»).
/// Значения кэшируются локально — офлайн-старт показывает последний бренд.
public class BrandingData
{
    [JsonPropertyName("appName")] public string AppName { get; set; } = "Заказ такси";
    [JsonPropertyName("appCode")] public string AppCode { get; set; } = "TaxiClient";
...
public static class BrandingService
{
    ...
    private static string CachePath =>
        Path.Combine(FileSystem.AppDataDirectory, "branding-client.json");
    ...
    public static async Task<BrandingData> LoadAsync()
    {
        try
        {
            var brand = await Http.GetFromJsonAsync<BrandingData>("branding.php?app=client");
            if (brand != null)
            {
    ...
    /// сервер отдаёт относительный путь вида /api/branding-logo.php?app=client&v=...
